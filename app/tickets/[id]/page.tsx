  ...
  return (
    <main className="min-h-screen bg-paper px-6 py-16 text-ink sm:px-10 lg:px-16">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="mt-8">
          <p className="text-sm text-sage">Ticket ID</p>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <p className="text-sm text-sage">Status</p>
            <p className="mt-1 capitalize text-ink">
              {formatStatusLabel(ticket.status)}
            </p>
          </div>
          <div className="sm:col-span-2">
            <p className="text-sm text-sage">Submitted by</p>
        ...
        <section className="mt-10 border-t border-ink/10 pt-8">
          ...
          <p className="mt-4 whitespace-pre-wrap text-2xl leading-relaxed text-ink sm:text-3xl">
            {ticket.description}
          </p>
        </section>

        <section className="mt-10 border-t border-ink/10 pt-8">
          <h2 className="text-sm font-normal text-sage">Conversations</h2>
          <ul className="mt-6 space-y-4">
            {ticket.comments.length === 0 ? (
              <li className="text-sm text-sage">No replies yet.</li>
            ) : (
        ...
        <section className="mt-10 border-t border-ink/10 pt-8">
          <h2 className="text-sm font-normal text-sage">Activity</h2>
          <ul className="mt-6 space-y-4">
            {ticket.activities.length === 0 ? (
              <li className="text-sm text-sage">No activity yet.</li>
            ) : (
